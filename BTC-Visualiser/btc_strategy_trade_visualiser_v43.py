# BTC Strategy Trade Visualiser (v4.8)
# Upgrades:
...
#
# Streamlit Cloud main file: BTC-Visualiser/btc_strategy_trade_visualiser_v43.py
...
st.set_page_config(page_title="BTC Strategy Trade Visualiser (v4.8)", layout="wide")
...
def roc(s, n): return s.pct_change(n) * 100.0
...
# Default exit model per strategy
# - AUTO maps to one of these based on strategy
...
def compute_signals(
    d: pd.DataFrame,
    strategy_name: str,
    fast: int, slow: int, trend_len: int,
    rsi_len: int, roc_len: int, vol_len: int,
    range_len: int, break_close: bool,
    bb_len: int, bb_k: float, squeeze_pct: float,
    use_session_vwap: bool, vwap_dev: float,
    ...
    """
    Returns df with:
      - long_sig, short_sig, sig_tag
      - score (0..100)
      - score_parts_* (debug columns)
      - indicators used by selected strategy
    """
    x = d.copy()

    # base indicators used across scoring
    ...
    if strategy_name == "MA Crossover":
        ...
        # confluence parts: trend bias + separation + RSI confirm
        parts = pd.DataFrame({
            "trend": ((x["close"] > x["TREND"]).astype(float)),
            "rsi": ((x["RSI"] - 50.0) / 50.0).clip(0, 1),  # >50 helps long
        ...
        parts = pd.DataFrame({
            "roc": (x["ROC"].abs() / max(1e-9, Xthr)).clip(0, 2) / 2,   # stronger ROC => higher
        ...
        # confluence: compression proxy (ATR% low) + breakout strength (distance)
        x["ATR14"] = atr(x, 14)
        ...
        parts = pd.DataFrame({
            "rsi": ((50.0 - x["RSI"]).clip(0, 50) / 50.0).fillna(0)  # deeper oversold => higher
                      .where(long_raw, ((x["RSI"] - 50.0).clip(0, 50) / 50.0).fillna(0)),
            "vol": (1.0 - x["VOL_EXP"].astype(float)),  # exhaustion proxy
...
def build_ml_features(df: pd.DataFrame):
    ...
    # include confluence score + RSI/ROC as features (if present)
    if "score" in d.columns:
        d["score_n"] = (d["score"] / 100.0).clip(0, 1)
    else:
        d["score_n"] = 0.0
    if "RSI" in d.columns:
        d["rsi_n"] = (d["RSI"] / 100.0).clip(0, 1)
    else:
        d["rsi_n"] = np.nan
    if "ROC" in d.columns:
        d["roc_n"] = (d["ROC"].clip(-5, 5) + 5) / 10.0
    else:
        d["roc_n"] = np.nan
...
# =====================
# BACKTEST (with exit models + ML gating)
# =====================
def backtest(
    df,
    atr_len, atr_mult, rr, risk,
    both_dirs, exit_on_opp,
    fill_next_open=True,
    ...
    for i in range(start_i, end_limit + 1):
        ...
        # EXIT / MANAGEMENT
            ...
            # exit model specific triggers
            ema_flip_hit = False
            time_hit = False

            if exit_model == "EMA Flip Exit":
                # proxy for "trendline break": EMA20 vs EMA50 flip
            ...
            # Opposite signal model (exit on opposite)
            if exit_model == "Opposite Signal":
                # if not already enabled, enforce it here
                if pos == 1:
                    opp_hit = short_sig
                else:
                    opp_hit = long_sig
            ...
            if should_exit:
                # intrabar tie-break only applies to TP/SL collisions
                ...
                # R multiple using original SL (stable)
                if pos == 1:
                    rdist = entry - sl
...
def run_backtest_cached(
    ...
    exit_model, trail_atr_mult, time_stop_bars,
...
# WALK-FORWARD ML
# =====================
def walk_forward_proba(d_sig: pd.DataFrame, X_all: np.ndarray, train_bars: int, test_bars: int,
                       atr_len: int, atr_mult: float, rr: float, risk: float,
                       both_dirs: bool, exit_on_opp: bool,
                       fill_next_open: bool, intrabar_rule: str,
                       exit_model: str, trail_atr_mult: float, time_stop_bars: int):
    """
    Rolling train/test:
      - Train on [t-train_bars, t) => build trades + labels
      - Predict proba on [t, t+test_bars)
    Produces proba array with values only in test slices (else NaN).
    """
    n = len(d_sig)
    ...
    while t < n - 50:
        ...
        # train labels from TP/SL only
        _, tt = backtest(
            d_train,
            atr_len=atr_len, atr_mult=atr_mult, rr=rr, risk=risk,
            both_dirs=both_dirs, exit_on_opp=exit_on_opp,
            fill_next_open=fill_next_open, intrabar_rule=intrabar_rule,
            exit_model=exit_model, trail_atr_mult=trail_atr_mult, time_stop_bars=time_stop_bars,
            use_ml=False
        )
        tt = tt[tt["outcome"].isin(["TP", "SL", "TRAIL_SL"])].copy()
        if len(tt) < 120:
            # not enough labels -> skip this window
            t = test_end
            continue

        # map trade signal_time to row indices in train slice
...
def add_session_overlays(fig, dts: pd.Series, show_asia=True, show_london=True, show_ny=True):
    ...
    # light transparent rectangles (no explicit colors requested by user; Plotly defaults are fine)
    sessions = []
    if show_asia:
...
# UI
# =====================
st.title("BTC Strategy Trade Visualiser (v4.8) — strategies + scoring + walk-forward ML")
...
with st.sidebar:
    ...
    strategy_name = st.selectbox("Choose strategy", STRATEGIES, index=0)
    ...
    roc_len = st.number_input("ROC Length", 2, 200, 20)
    momentum_roc_thresh = st.number_input("Momentum ROC threshold (%)", 0.05, 5.0, 0.25, 0.05)
    vol_len = st.number_input("Volume MA Length", 2, 500, 20)

    range_len = st.number_input("Range lookback", 5, 500, 50)
    break_close = st.toggle("Range breakout requires CLOSE outside", True)

    bb_len = st.number_input("BB Length", 5, 300, 20)
    bb_k = st.number_input("BB StdDev", 1.0, 4.0, 2.0, 0.25)
    squeeze_pct = st.number_input("Squeeze threshold (BB width %)", 0.1, 20.0, 2.0, 0.1)

    use_session_vwap = st.toggle("VWAP: session reset (daily)", True)
    vwap_dev = st.number_input("VWAP deviation trigger (%)", 0.1, 10.0, 1.0, 0.1)

    st.divider()
    st.header("Confluence Scoring")
    st.caption("Signals only taken if score ≥ min score. Score = weighted average of components.")
    min_score = st.slider("Min score to take trade", 0.0, 100.0, 55.0, 1.0)

    # Weights (shared across strategies; unused parts are ignored)
    w_trend = st.slider("Weight: Trend", 0.0, 5.0, 2.0, 0.5)
    w_rsi = st.slider("Weight: RSI", 0.0, 5.0, 1.5, 0.5)
    w_vol = st.slider("Weight: Volume", 0.0, 5.0, 1.5, 0.5)
    w_strength = st.slider("Weight: Strength/Distance", 0.0, 5.0, 1.5, 0.5)
    w_compression = st.slider("Weight: Compression", 0.0, 5.0, 1.0, 0.5)
    w_pullback = st.slider("Weight: Pullback", 0.0, 5.0, 1.0, 0.5)
    w_roc = st.slider("Weight: ROC", 0.0, 5.0, 2.0, 0.5)
    w_wick = st.slider("Weight: Wick/Rejection", 0.0, 5.0, 1.0, 0.5)
    w_dev = st.slider("Weight: Deviation", 0.0, 5.0, 2.0, 0.5)
    ...
    exit_on_opp = st.toggle("Exit on opposite signal (extra)", False)

    fill_next_open = st.toggle("Enter on next candle OPEN (more realistic)", True)
    intrabar_rule = st.selectbox("If TP & SL hit same candle", ["Worst case (SL first)", "Best case (TP first)"], index=0)

    st.divider()
    st.header("Exit Model")
    exit_override = st.selectbox("Exit model", EXIT_MODELS, index=0)
    trail_atr_mult = st.number_input("Trailing ATR multiple", 0.5, 10.0, 2.0, 0.25)
    time_stop_bars = st.number_input("Time stop (bars)", 5, 5000, 48)

    st.caption("AUTO = best-fit exit per strategy (momentum=trail, trend=EMA flip, crossover=opp signal, reversion=time).")
    ...
    train_frac = st.slider("Single-split training fraction", 0.3, 0.9, 0.7)

    wf_train_bars = st.number_input("Walk-forward train bars", 200, 20000, 3000)
    wf_test_bars = st.number_input("Walk-forward test bars", 50, 5000, 500)

    proba_thresh = st.slider("Only take trades if P(win) ≥", 0.50, 0.80, 0.55, 0.01)

    st.divider()
    st.header("Chart")
    show_indicators = st.toggle("Show strategy indicator lines", True)
    ...
    show_sessions = st.toggle("Show session overlays", True)
...
# Resolve exit model (AUTO -> strategy default)
exit_model = exit_override
if exit_model == "AUTO":
    exit_model = DEFAULT_EXIT_FOR_STRAT.get(strategy_name, "TP/SL only")

# =====================
# Compute signals + confluence score
# =====================
d_sig = compute_signals(
    df_tf,
    strategy_name=strategy_name,
    fast=int(fast), slow=int(slow), trend_len=int(trend_len),
    rsi_len=int(rsi_len), roc_len=int(roc_len), vol_len=int(vol_len),
    range_len=int(range_len), break_close=bool(break_close),
    bb_len=int(bb_len), bb_k=float(bb_k), squeeze_pct=float(squeeze_pct),
    use_session_vwap=bool(use_session_vwap), vwap_dev=float(vwap_dev),
...
# =====================
# ML proba (single-split or walk-forward)
# =====================
proba = None
ml_info = None
...
if use_ml:
    ...
    if ml_mode == "Walk-forward (realistic)":
        proba, nwin, ntr = walk_forward_proba(
            ...
            atr_len=int(atr_len), atr_mult=float(atr_mult), rr=float(rr), risk=float(risk),
            both_dirs=bool(both_dirs), exit_on_opp=bool(exit_on_opp),
            fill_next_open=bool(fill_next_open), intrabar_rule=str(intrabar_rule),
            exit_model=str(exit_model), trail_atr_mult=float(trail_atr_mult), time_stop_bars=int(time_stop_bars)
        )
        ml_info = f"Walk-forward ML: windows trained={nwin}, total labeled trades used={ntr}. Features: {', '.join(feat_cols)}"
    else:
        # single split
        ...
        # labels from training trades (TP/SL only)
        _, tt = backtest(
            d_train,
            atr_len=int(atr_len), atr_mult=float(atr_mult), rr=float(rr), risk=float(risk),
            both_dirs=bool(both_dirs), exit_on_opp=bool(exit_on_opp),
...
# =====================
# Backtest (with optional ML gating)
# =====================
d_full, trades = run_backtest_cached(
    d_sig,
    int(atr_len), float(atr_mult), float(rr), float(risk),
    bool(both_dirs), bool(exit_on_opp),
    bool(fill_next_open), str(intrabar_rule),
    str(exit_model), float(trail_atr_mult), int(time_stop_bars),
...
if not trades.empty:
    ...
    labels = [
        f"#{i+1} {r.side} | {r.entry_time:%Y-%m-%d %H:%M} → {r.exit_time:%Y-%m-%d %H:%M} | {r.outcome} | Score {r.score:.0f} | P(win) {r.p_win:.2f} | R {r.r_mult:+.2f} | {r.sig_tag} | {r.exit_model}"
...
# Indicator lines (only if present)
if show_indicators:
...
# ML P(win) overlay on signal points
